...
public final class GoogleGeocoder
{
    ...
    private static final String PREFERENCES_GEOCODER = GoogleGeocoder.class.getName()
            + ".GEOCODER";
    private static final String KEY_ALLOW = GoogleGeocoder.class.getName()
            + ".KEY_ALLOW";
            ...
            if (status.equals(STATUS_OK)) {
                ...
                for (int i = 0; i < maxResults && i < a.length(); i++) {
                    ...
                    for(int j = 0; j < addressComponents.length(); j++) {
                        JSONObject d = addressComponents.getJSONObject(j);
                        String type = d.getJSONArray("types").getString(0);
                        String longName = d.getString("long_name");
